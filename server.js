...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    `      } else {`,
    `        # Extract just the [ExtraBrowserDocks] section so size= is scoped correctly`,
    `        # (other sections may also have size= keys that must not be matched)`,
    `        $secPat   = '(?ms)(^\\[ExtraBrowserDocks\\].*?)(?=\\r?\\n\\[|\\z)'`,
    `        $secMatch = [regex]::Match($txt, $secPat)`,
    `        if ($secMatch.Success) {`,
    `          $sec    = $secMatch.Value`,
    `          $szM    = [regex]::Match($sec, '(?m)^size=(\\d+)')`,
    `          $n      = if ($szM.Success) { [int]$szM.Groups[1].Value + 1 } else { 1 }`,
    `          $entry  = "$n\\Title=ESOC Docker\`r\`n$n\\Url=$url\`r\`n"`,
    `          $newSec = [regex]::Replace($sec, '(?m)^size=\\d+', ($entry + "size=$n"))`,
    `          $txt    = [regex]::Replace($txt, $secPat, $newSec)`,
    `        } else {`,
    `          $txt = $txt.TrimEnd() + "\`r\`n\`r\`n[ExtraBrowserDocks]\`r\`n1\\Title=ESOC Docker\`r\`n1\\Url=$url\`r\`nsize=1\`r\`n"`,
    `        }`,
