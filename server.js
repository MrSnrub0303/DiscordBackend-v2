...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    `Write-Host '[2/3] Writing ESOC Docker dock JSON to OBS ini file(s)...'`,
    ...
    `    if ($txt -match '(?m)^ExtraBrowserDocks=(.*)') { Write-Host "    BEFORE: $($Matches[1])" -ForegroundColor DarkCyan } else { Write-Host '    BEFORE: (no ExtraBrowserDocks key)' -ForegroundColor DarkCyan }`,
    `    # Strip any existing ExtraBrowserDocks line from wherever it appears`,
    `    $txt = [regex]::Replace($txt, '(?m)^ExtraBrowserDocks=.*\r?\n?', '')`,
    `    # Inject into [BasicWindow], creating the section if missing`,
    `    $bwPat = '(?ms)^\\[BasicWindow\\].*?(?=\\r?\\n\\[|\\z)'`,
    `    $bwM   = [regex]::Match($txt, $bwPat)`,
    `    if ($bwM.Success) {`,
    `      $newSec = $bwM.Value.TrimEnd() + "\`r\`nExtraBrowserDocks=$newDocks"`,
    ...
    `    Write-Host "    AFTER:  $newDocks" -ForegroundColor Green`,
    `  } catch {`,
