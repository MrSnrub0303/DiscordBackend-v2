...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    ``,
    `$url      = '${dashboardUrl}'`,
    `$newEntry = "{""name"":""ESOC Docker"",""url"":""$url""}"`,
    ``,
    `# Step 2: global.ini [BasicWindow] ExtraBrowserDocks JSON (OBS 28+ primary location)`,
    `Write-Host '[2/3] Adding dock to global.ini [BasicWindow]...'`,
    `try {`,
    `  $txt = [IO.File]::ReadAllText($globalIni)`,
    `  if ($txt -match 'ExtraBrowserDocks=.*ESOC Docker') {`,
    `    Write-Host '[OK] ESOC Docker already present in global.ini [BasicWindow].' -ForegroundColor Green`,
    `  } else {`,
    `    $bwPat   = '(?ms)^\\[BasicWindow\\].*?(?=\\r?\\n\\[|\\z)'`,
    `    $bwMatch = [regex]::Match($txt, $bwPat)`,
    `    if ($bwMatch.Success) {`,
    `      $sec = $bwMatch.Value`,
    `      $dlM = [regex]::Match($sec, '(?m)^ExtraBrowserDocks=(.*)')`,
    `      if ($dlM.Success) {`,
    `        $existing = $dlM.Groups[1].Value.Trim()`,
    `        $li = $existing.LastIndexOf(']')`,
    `        $newDocks = if ($li -le 0) { "[$newEntry]" } else { $existing.Substring(0,$li) + ",$newEntry]" }`,
    `        $newSec = [regex]::Replace($sec, '(?m)^ExtraBrowserDocks=.*', "ExtraBrowserDocks=$newDocks")`,
    `      } else {`,
    `        $newDocks = "[$newEntry]"`,
    `        $newSec   = $sec.TrimEnd() + "\`r\`nExtraBrowserDocks=$newDocks"`,
    `      }`,
    `      $txt = $txt.Replace($bwMatch.Value, $newSec)`,
    `    } else {`,
    `      $newDocks = "[$newEntry]"`,
    `      $txt = $txt.TrimEnd() + "\`r\`n\`r\`n[BasicWindow]\`r\`nExtraBrowserDocks=$newDocks\`r\`n"`,
    `    }`,
    `    [IO.File]::WriteAllText($globalIni, $txt, $utf8)`,
    `    Write-Host "[OK] ExtraBrowserDocks => $newDocks" -ForegroundColor Green`,
    `  }`,
    `} catch {`,
    `  Write-Host "[FAIL] global.ini [BasicWindow]: $_" -ForegroundColor Red`,
    `}`,
    ``,
    `# Step 3: ALL profile basic.ini files — flat libobs format (belt+suspenders)`,
    `Write-Host '[3/3] Adding dock to all profile basic.ini files...'`,
    ...
    `  Write-Host "  Found $($profDirs.Count) profile(s): $(($profDirs | ForEach-Object { $_.Name }) -join ', ')" -ForegroundColor DarkGray`,
    `  foreach ($profDir in $profDirs) {`,
    `    $basicIni = Join-Path $profDir.FullName 'basic.ini'`,
    `    Write-Host "  Profile '$($profDir.Name)'..." -ForegroundColor DarkGray`,
    ...
    `      if ($txt -match '(?m)^ExtraBrowserDock\\d+Title=ESOC Docker') {`,
    `        Write-Host '    [OK] Already configured.' -ForegroundColor Green`,
    `      } else {`,
    `        $docks = @()`,
    `        if ($sM.Success) {`,
    `          $sec = $sM.Value`,
    `          $cM  = [regex]::Match($sec, '(?m)^ExtraBrowserDockCount=(\\d+)')`,
    `          $cnt = if ($cM.Success) { [int]$cM.Groups[1].Value } else { 0 }`,
    `          for ($i = 0; $i -lt $cnt; $i++) {`,
    `            $nM = [regex]::Match($sec, "(?m)^ExtraBrowserDock$($i)Title=(.+)")`,
    `            $uM = [regex]::Match($sec, "(?m)^ExtraBrowserDock$($i)Url=(.+)")`,
    `            if ($nM.Success -and $uM.Success) { $docks += @{ N=$nM.Groups[1].Value.TrimEnd(); U=$uM.Groups[1].Value.TrimEnd() } }`,
    `          }`,
    `        }`,
    `        $docks  += @{ N='ESOC Docker'; U=$url }`,
    `        $txt     = [regex]::Replace($txt, $sPat, '').TrimEnd()`,
    `        $newSec  = "[ExtraBrowserDocks]\`r\`nExtraBrowserDockCount=$($docks.Count)\`r\`n"`,
    `        for ($i = 0; $i -lt $docks.Count; $i++) {`,
    `          $newSec += "ExtraBrowserDock$($i)Title=$($docks[$i].N)\`r\`n"`,
    `          $newSec += "ExtraBrowserDock$($i)Url=$($docks[$i].U)\`r\`n"`,
    `        }`,
    `        $txt += "\`r\`n\`r\`n" + $newSec`,
    `        [IO.File]::WriteAllText($basicIni, $txt, $utf8)`,
    `        Write-Host '    [OK] Dock added.' -ForegroundColor Green`,
    `      }`,
    `    } catch {`,
