...
function buildObsSetupBat(dashboardUrl, serverBaseUrl) {
  ...
  const psLines = [
    ...
    `    # bounds_type 2 = SCALE_INNER: AoE3 borderless/fullscreen on 4K presents swap chain at 3840x2160 even when in-game res is 1920x1080 — without bounds the canvas shows only the top-left crop`,
    `    try {`,
    `      $sceneObj = $json | ConvertFrom-Json`,
    `      $gcNames  = @(); $gcUuids = @()`,
    `      foreach ($src in $sceneObj.sources) {`,
    `        if ($src.id -eq 'game_capture') {`,
    `          if ($src.name) { $gcNames += [string]$src.name }`,
    `          if ($src.uuid) { $gcUuids += [string]$src.uuid }`,
    `        }`,
    `      }`,
    `      $patched = 0`,
    `      foreach ($src in $sceneObj.sources) {`,
    `        if ($src.id -eq 'scene' -and $src.settings -and $src.settings.items) {`,
    `          foreach ($item in $src.settings.items) {`,
    `            $isGC = ($item.source_uuid -and ($gcUuids -contains [string]$item.source_uuid)) -or`,
    `                    ($item.name -and ($gcNames -contains [string]$item.name))`,
    `            if ($isGC) {`,
    `              $item.bounds_type = 2`,
    `              $item.bounds.x    = 1920.0`,
    `              $item.bounds.y    = 1080.0`,
    `              $item.pos.x       = 0.0`,
    `              $item.pos.y       = 0.0`,
    `              $patched++`,
    `            }`,
    ...
    `        }`,
    `      }`,
    `      if ($patched -gt 0) { $json = $sceneObj | ConvertTo-Json -Depth 100; Write-Host "  DPI fix: bounds patched on $patched game capture item(s)." -ForegroundColor DarkGray }`,
    ...
    `  # Set ESOC Profile as the active profile in [General]`,
    `  if ($txt -match '(?m)^CurrentProfile=') {`,
    `    $txt = [regex]::Replace($txt, '(?m)^CurrentProfile=.*$', 'CurrentProfile=ESOC Profile')`,
    `  } else {`,
    `    $gM = [regex]::Match($txt, '(?ms)^\\[General\\].*?(?=\\r?\\n\\[|\\z)')`,
    `    if ($gM.Success) { $txt = $txt.Replace($gM.Value, $gM.Value.TrimEnd() + "\`r\`nCurrentProfile=ESOC Profile") }`,
    `  }`,
    `  if ($txt -match '(?m)^CurrentProfileDir=') {`,
    `    $txt = [regex]::Replace($txt, '(?m)^CurrentProfileDir=.*$', 'CurrentProfileDir=ESOC Profile')`,
    `  } else {`,
    `    $gM = [regex]::Match($txt, '(?ms)^\\[General\\].*?(?=\\r?\\n\\[|\\z)')`,
    `    if ($gM.Success) { $txt = $txt.Replace($gM.Value, $gM.Value.TrimEnd() + "\`r\`nCurrentProfileDir=ESOC Profile") }`,
    `  }`,
    ...
    `    '[Output]', 'Mode=Simple', '',`,
    `    '[SimpleOutput]', 'VBitrate=6000', "StreamEncoder=$simpleEnc", '',`,
    `    '[AdvOut]', "Encoder=$advEnc", '',`,
    `    '[Video]', 'BaseCX=1920', 'BaseCY=1080', 'OutputCX=1920', 'OutputCY=1080', 'FPSType=0', 'FPSCommon=60.00', '',`,
    `    '[ExtraBrowserDocks]', 'ExtraBrowserDockCount=1', 'ExtraBrowserDock0Title=ESOC Docker', "ExtraBrowserDock0Url=$url", ''`,
    `  )`,
    `  $basicContent = $basicLines -join "\`r\`n"`,
    `  [IO.File]::WriteAllText($basicIni, $basicContent, $utf8)`,
    `  Write-Host "[OK] ESOC Profile: StreamEncoder=$simpleEnc, canvas 1920x1080 @ 60fps, dock written." -ForegroundColor Green`,
    `} catch {`,
