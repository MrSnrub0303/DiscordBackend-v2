...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    `    Write-Host "  basic.ini path: $basicIni" -ForegroundColor DarkGray`,
    `    try {`,
    `      $txt = [IO.File]::ReadAllText($basicIni)`,
    `      $url = '${dashboardUrl}'`,
    `      $secPat   = '(?ms)^\\[ExtraBrowserDocks\\].*?(?=\\r?\\n\\[|\\z)'`,
    `      $secMatch = [regex]::Match($txt, $secPat)`,
    `      Write-Host ''`,
    `      Write-Host '── DEBUG: [ExtraBrowserDocks] section BEFORE ──────────────' -ForegroundColor DarkCyan`,
    `      if ($secMatch.Success) { Write-Host $secMatch.Value -ForegroundColor DarkCyan } else { Write-Host '(section not found)' -ForegroundColor DarkCyan }`,
    `      Write-Host '───────────────────────────────────────────────────────────' -ForegroundColor DarkCyan`,
    `      Write-Host ''`,
    `      if ($txt -match '(?m)^DockName\\d+=ESOC Docker') {`,
    `        Write-Host '[OK] ESOC Docker dock already in new format.' -ForegroundColor Green`,
    `      } else {`,
    `        $docks = @()`,
    `        if ($secMatch.Success) {`,
    `          $sec  = $secMatch.Value`,
    `          $cntM = [regex]::Match($sec, '(?m)^count=(\\d+)')`,
    `          $cnt  = if ($cntM.Success) { [int]$cntM.Groups[1].Value } else { 0 }`,
    `          Write-Host "  Existing dock count in file: $cnt" -ForegroundColor DarkGray`,
    `          for ($i = 0; $i -lt $cnt; $i++) {`,
    `            $nM = [regex]::Match($sec, "(?m)^DockName$i=(.+)")`,
    `            $uM = [regex]::Match($sec, "(?m)^DockUrl$i=(.+)")`,
    `            if ($nM.Success -and $uM.Success) { $docks += @{ N=$nM.Groups[1].Value.TrimEnd(); U=$uM.Groups[1].Value.TrimEnd() } }`,
    `          }`,
    `        }`,
    `        $docks += @{ N='ESOC Docker'; U=$url }`,
    `        $newSec  = "[ExtraBrowserDocks]\`r\`ncount=$($docks.Count)\`r\`n"`,
    `        for ($i = 0; $i -lt $docks.Count; $i++) { $newSec += "DockName$i=$($docks[$i].N)\`r\`nDockUrl$i=$($docks[$i].U)\`r\`n" }`,
    `        Write-Host '── DEBUG: section being written ───────────────────────────' -ForegroundColor DarkYellow`,
    `        Write-Host $newSec -ForegroundColor DarkYellow`,
    `        Write-Host '───────────────────────────────────────────────────────────' -ForegroundColor DarkYellow`,
    `        Write-Host ''`,
    `        $txt = [regex]::Replace($txt, $secPat, '').TrimEnd()`,
    `        $txt += "\`r\`n\`r\`n" + $newSec`,
    `        [IO.File]::WriteAllText($basicIni, $txt, $utf8)`,
    `        $verify = [IO.File]::ReadAllText($basicIni)`,
    `        $vMatch = [regex]::Match($verify, $secPat)`,
    `        Write-Host '── DEBUG: [ExtraBrowserDocks] section AFTER write ─────────' -ForegroundColor DarkCyan`,
    `        if ($vMatch.Success) { Write-Host $vMatch.Value -ForegroundColor DarkCyan } else { Write-Host '(section not found after write!)' -ForegroundColor Red }`,
    `        Write-Host '───────────────────────────────────────────────────────────' -ForegroundColor DarkCyan`,
    `        Write-Host ''`,
    `        Write-Host '[OK] ESOC Docker dock written.' -ForegroundColor Green`,
    `      }`,
    `    } catch {`,
    ...
    `Write-Host ''`,
    `Write-Host '==================================================='`,
    `Write-Host ' Debug complete. Screenshot this window and share it.' -ForegroundColor Yellow`,
    `Write-Host '==================================================='`,
  ];
