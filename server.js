...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    `  } else {`,
    `    # ── Step 3: Browser dock (OBS 28+ format: count/DockName0/DockUrl0) ──`,
    ...
    `      } else {`,
    `        # Read existing docks from current section (new OBS 28+ format only)`,
    ...
    `        # Remove old section (any format) and append the new one`,
