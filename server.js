...
function buildObsSetupBat(dashboardUrl, serverBaseUrl) {
  ...
  const psLines = [
    ...
    `}`,
    ``,
    `# Step 6: ALL profile basic.ini files — encoder, bitrate, dock`,
    `Write-Host '[6/6] Configuring encoder + dock for all profiles...'`,
    `$profilesDir = Join-Path $obsRoot 'basic\\profiles'`,
    `if (-not (Test-Path $profilesDir)) {`,
    `  Write-Host '  [WARN] No profiles directory found.' -ForegroundColor Yellow`,
    `} else {`,
    `  # Detect GPU and select the matching OBS encoder IDs`,
    ...
    `  Write-Host "  Encoder: $simpleEnc" -ForegroundColor DarkGray`,
    `  $profDirs = @(Get-ChildItem -Path $profilesDir -Directory)`,
    `  Write-Host "  Profiles: $(($profDirs | ForEach-Object { $_.Name }) -join ', ')" -ForegroundColor DarkGray`,
    `  foreach ($profDir in $profDirs) {`,
    `    $basicIni = Join-Path $profDir.FullName 'basic.ini'`,
    `    Write-Host "  '$($profDir.Name)'" -ForegroundColor DarkGray`,
    `    if (-not (Test-Path $basicIni)) { Write-Host '    (no basic.ini, skipping)' -ForegroundColor DarkGray; continue }`,
    `    try {`,
    `      $txt  = [IO.File]::ReadAllText($basicIni)`,
    `      # Encoder + bitrate (only update keys that already exist — safe, no injection)`,
    `      if ($txt -match '(?m)^VBitrate=')      { $txt = [regex]::Replace($txt, '(?m)^VBitrate=.*$',      'VBitrate=6000') }`,
    `      if ($txt -match '(?m)^StreamEncoder=') { $txt = [regex]::Replace($txt, '(?m)^StreamEncoder=.*$', "StreamEncoder=$simpleEnc") }`,
    `      if ($txt -match '(?m)^Encoder=')       { $txt = [regex]::Replace($txt, '(?m)^Encoder=.*$',       "Encoder=$advEnc") }`,
    `      # Dock entry`,
    `      $sPat = '(?ms)^\\[ExtraBrowserDocks\\].*?(?=\\r?\\n\\[|\\z)'`,
    `      $sM   = [regex]::Match($txt, $sPat)`,
    `      if ($sM.Success) { Write-Host "    BEFORE: $($sM.Value -replace '\r','' -replace '\n','|')" -ForegroundColor DarkCyan } else { Write-Host '    BEFORE: (no [ExtraBrowserDocks])' -ForegroundColor DarkCyan }`,
    `      $txt    = [regex]::Replace($txt, $sPat, '').TrimEnd()`,
    `      $newSec = "[ExtraBrowserDocks]\`r\`nExtraBrowserDockCount=1\`r\`nExtraBrowserDock0Title=ESOC Docker\`r\`nExtraBrowserDock0Url=$url\`r\`n"`,
    `      $txt   += "\`r\`n\`r\`n" + $newSec`,
    `      [IO.File]::WriteAllText($basicIni, $txt, $utf8)`,
    `      Write-Host "    [OK] StreamEncoder=$simpleEnc VBitrate=6000 + dock written." -ForegroundColor Green`,
    `    } catch {`,
    `      Write-Host "    [FAIL] $($profDir.Name): $_" -ForegroundColor Red`,
    `    }`,
    `  }`,
    `}`,
    ``,
