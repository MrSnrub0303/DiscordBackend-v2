...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    `  } else {`,
    `    # ── Step 3: Browser dock (Qt INI format: size / N\\Title / N\\Url) ────`,
    `    Write-Host '[3/3] Adding ESOC Docker browser dock...'`,
    `    Write-Host "  File: $basicIni" -ForegroundColor DarkGray`,
    `    try {`,
    `      $txt = [IO.File]::ReadAllText($basicIni)`,
    `      $url = '${dashboardUrl}'`,
    `      $secPat   = '(?ms)^\\[ExtraBrowserDocks\\].*?(?=\\r?\\n\\[|\\z)'`,
    `      $secMatch = [regex]::Match($txt, $secPat)`,
    `      Write-Host '--- BEFORE ---' -ForegroundColor DarkCyan`,
    `      if ($secMatch.Success) { Write-Host ($secMatch.Value -replace '\r','') -ForegroundColor DarkCyan } else { Write-Host '(no [ExtraBrowserDocks] section)' -ForegroundColor DarkCyan }`,
    `      if ($txt -match '(?m)^\\d+\\\\Title=ESOC Docker') {`,
    `        Write-Host '[OK] ESOC Docker dock already configured.' -ForegroundColor Green`,
    `      } else {`,
    `        $docks = @()`,
    `        if ($secMatch.Success) {`,
    `          $sec = $secMatch.Value`,
    `          $szM = [regex]::Match($sec, '(?m)^size=(\\d+)')`,
    `          $cnt = if ($szM.Success) { [int]$szM.Groups[1].Value } else { 0 }`,
    `          for ($i = 1; $i -le $cnt; $i++) {`,
    `            $nM = [regex]::Match($sec, "(?m)^$i\\\\Title=(.+)")`,
    `            $uM = [regex]::Match($sec, "(?m)^$i\\\\Url=(.+)")`,
    `            if ($nM.Success -and $uM.Success) { $docks += @{ N=$nM.Groups[1].Value.TrimEnd(); U=$uM.Groups[1].Value.TrimEnd() } }`,
    `          }`,
    `        }`,
    `        $docks += @{ N='ESOC Docker'; U=$url }`,
    `        $newSec  = "[ExtraBrowserDocks]\`r\`nsize=$($docks.Count)\`r\`n"`,
    `        for ($i = 0; $i -lt $docks.Count; $i++) {`,
    `          $idx     = $i + 1`,
    `          $newSec += "$idx\\Title=$($docks[$i].N)\`r\`n$idx\\Url=$($docks[$i].U)\`r\`n"`,
    `        }`,
    `        Write-Host '--- WRITING ---' -ForegroundColor DarkYellow`,
    `        Write-Host ($newSec -replace '\r','') -ForegroundColor DarkYellow`,
    `        $txt = [regex]::Replace($txt, $secPat, '').TrimEnd()`,
    `        $txt += "\`r\`n\`r\`n" + $newSec`,
    `        [IO.File]::WriteAllText($basicIni, $txt, $utf8)`,
    `        $after = [IO.File]::ReadAllText($basicIni)`,
    `        $aMatch = [regex]::Match($after, $secPat)`,
    `        Write-Host '--- AFTER ---' -ForegroundColor DarkCyan`,
    `        if ($aMatch.Success) { Write-Host ($aMatch.Value -replace '\r','') -ForegroundColor DarkCyan } else { Write-Host '(section missing after write!)' -ForegroundColor Red }`,
