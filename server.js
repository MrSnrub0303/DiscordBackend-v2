...
function buildObsSetupBat(dashboardUrl, serverBaseUrl) {
  ...
  const psLines = [
    ...
    `      $json = [regex]::Replace($json, '(?s)("bounds_type":\\s*)0(?=(?:(?!"bounds_type":).)*?"source_uuid":\\s*"' + $gcUuid + '")', '\${1}2')`,
    `      $json = [regex]::Replace($json, '(?s)("bounds":\\s*\\{[^}]*?"x":\\s*)[\\d.]+([^}]*?"y":\\s*)[\\d.]+([^}]*?\\})(?=(?:(?!"bounds":).)*?"source_uuid":\\s*"' + $gcUuid + '")', '\${1}1920.0\${2}1080.0\${3}')`,
