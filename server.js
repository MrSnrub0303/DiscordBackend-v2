...
function buildObsSetupBat(dashboardUrl) {
  ...
  const psLines = [
    ...
    `    # Strip old ExtraBrowserDocks and DockState lines (they will be re-injected below)`,
    `    $txt = [regex]::Replace($txt, '(?m)^ExtraBrowserDocks=.*\r?\n?', '')`,
    `    $txt = [regex]::Replace($txt, '(?m)^DockState=.*\r?\n?', '')`,
    `    # Inject both keys into [BasicWindow], creating the section if missing`,
    `    $bwPat = '(?ms)^\\[BasicWindow\\].*?(?=\\r?\\n\\[|\\z)'`,
    `    $bwM   = [regex]::Match($txt, $bwPat)`,
    `    if ($bwM.Success) {`,
    `      $newSec = $bwM.Value.TrimEnd() + "\`r\`nExtraBrowserDocks=$newDocks\`r\`nDockState=$dockState"`,
    `      $txt    = $txt.Replace($bwM.Value, $newSec)`,
    `    } else {`,
    `      $txt = $txt.TrimEnd() + "\`r\`n\`r\`n[BasicWindow]\`r\`nExtraBrowserDocks=$newDocks\`r\`nDockState=$dockState\`r\`n"`,
    `    }`,
    `    [IO.File]::WriteAllText($iniFile, $txt, $utf8)`,
    `    Write-Host "    [OK] ExtraBrowserDocks + DockState written." -ForegroundColor Green`,
    `  } catch {`,
